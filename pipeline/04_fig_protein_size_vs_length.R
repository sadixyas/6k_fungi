...
# 2) fetch total protein counts & avg length per genome
...
# 3) fetch secreted protein counts & avg length per genome
...
# 4) fetch PHYLUM from species table
species_meta <- dbReadTable(con, "species") %>%
  rename(locustag = LOCUSTAG) %>%
  select(locustag, PHYLUM) %>%
  distinct()

# 5) fetch trophicMode from funguild, renaming species_prefix → locustag
...
# 10) compute groupwise Pearson r
r_all_phylum   <- df_all %>%   group_by(PHYLUM)               %>% summarize(r = cor(avg_length, total_n, use="complete.obs"))
r_all_trophic  <- df_all %>%   group_by(Trophic_mode_grouped) %>% summarize(r = cor(avg_length, total_n, use="complete.obs"))
r_sec_phylum   <- df_sec %>%   group_by(PHYLUM)               %>% summarize(r = cor(avg_secreted_length, secreted_n, use="complete.obs"))
r_sec_trophic  <- df_sec %>%   group_by(Trophic_mode_grouped) %>% summarize(r = cor(avg_secreted_length, secreted_n, use="complete.obs"))
r_non_phylum   <- df_non %>%   group_by(PHYLUM)               %>% summarize(r = cor(avg_non_secreted_length, non_secreted_n, use="complete.obs"))
...
# 11) theme and annotation helpers
...
p_all_by_phylum <- ggplot(df_all, aes(avg_length, total_n, color = PHYLUM)) +
...
p_sec_by_phylum <- ggplot(df_sec, aes(avg_secreted_length, secreted_n, color = PHYLUM)) +
  geom_point(size=2, alpha=0.7) +
  geom_smooth(method="lm", se=FALSE, linetype="dashed") +
  add_group_r(r_sec_phylum) +
  facet_wrap(~PHYLUM, scales="free") +
  labs(title="Secreted Proteins Faceted by Phylum", x="Average secreted protein length (aa)", y="Secreted protein count") +
  base_theme
...
p_sec_by_trophic <- ggplot(df_sec, aes(avg_secreted_length, secreted_n, color = PHYLUM)) +
  ...
  facet_wrap(~Trophic_mode_grouped, scales="free") +
  labs(title="Secreted Proteins Faceted by Trophic Mode", x="Average secreted protein length (aa)", y="Secreted protein count") +
  base_theme
...
p_non_by_phylum <- ggplot(df_non, aes(avg_non_secreted_length, non_secreted_n, color = PHYLUM)) +
  geom_point(size=2, alpha=0.7) +
  geom_smooth(method="lm", se=FALSE, linetype="dashed") +
  add_group_r(r_non_phylum) +
  facet_wrap(~PHYLUM, scales="free") +
  labs(title="Non-Secreted Proteins Faceted by Phylum", x="Average non-secreted protein length (aa)", y="Non-secreted protein count") +
  base_theme
...
p_non_by_trophic <- ggplot(df_non, aes(avg_non_secreted_length, non_secreted_n, color = PHYLUM)) +
  ...
  facet_wrap(~Trophic_mode_grouped, scales="free") +
  labs(title="Non-Secreted Proteins Faceted by Trophic Mode", x="Average non-secreted protein length (aa)", y="Non-secreted protein count") +
  base_theme
